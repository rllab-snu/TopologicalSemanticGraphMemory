...
class RolloutStorage:
    ...
    def __init__(
        ...
        if action_space.__class__.__name__ == "ActionSpace" or action_space.__class__.__name__ == "Discrete" :
            action_shape = 1
        else:
            action_shape = action_space.n#.shape[0]
    ...
    def to(self, device):
        for sensor in self.observations:
            if sensor in self.MEMORY_LIST:
                self.observations[sensor] = self.observations[sensor].to("cpu")
            else:
    ...
    def insert(
        ...
        for sensor in observations:
            if sensor in self.OBS_LIST:
                if sensor in self.MEMORY_LIST:
                    aa = observations[sensor]
                    node_num = aa.shape[1]
                    if "A" in sensor:
                        node_num2 = aa.shape[2]
                        self.observations[sensor][self.step + 1][:, :node_num, :node_num2].copy_(
                            observations[sensor].cpu()
                        )
                    else:
                        self.observations[sensor][self.step + 1][:, :node_num].copy_(
                            observations[sensor].cpu()
                        )
    ...
    def recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            for sensor in observations_batch:
                ...
                if sensor in self.MEMORY_LIST:
                    observations_batch[sensor] = observations_batch[sensor].to(self.device)
