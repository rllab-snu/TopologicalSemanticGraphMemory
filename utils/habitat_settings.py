...
def make_cfg(settings):
    ...
    def create_pano_part_camera_spec(**kw_args):
        camera_sensor_spec = habitat_sim.CameraSensorSpec()
        camera_sensor_spec.sensor_type = habitat_sim.SensorType.COLOR
        camera_sensor_spec.resolution = [settings["pano_height"], settings["cam_width"]]
        camera_sensor_spec.position = [0, settings["sensor_height"], 0]
        for k in kw_args:
            setattr(camera_sensor_spec, k, kw_args[k])
        return camera_sensor_spec
    ...
    if settings["panoramic_sensor"]:
        num_of_camera = 12
        hfov = 360 / num_of_camera
        multiview_orientation = [2 * np.pi * idx / num_of_camera for idx in range(num_of_camera - 1, -1, -1)]
        half = num_of_camera // 2
        multiview_orientation = multiview_orientation[half:] + multiview_orientation[:half]
        for i, orient in enumerate(multiview_orientation):
            color_sensor_spec = create_pano_part_camera_spec(
                uuid="panoramic_rgb_part_sensor_{}".format(i),
                hfov=hfov,
                sensor_type=habitat_sim.SensorType.COLOR,
                sensor_subtype=habitat_sim.SensorSubType.PINHOLE,
            ...
            depth_sensor_spec = create_pano_part_camera_spec(
                uuid="panoramic_depth_part_sensor_{}".format(i),
                hfov=hfov,
                sensor_type=habitat_sim.SensorType.DEPTH,
                channels=1,
                sensor_subtype=habitat_sim.SensorSubType.PINHOLE,
            ...
            semantic_sensor_spec = create_pano_part_camera_spec(
                uuid="panoramic_semantic_part_sensor_{}".format(i),
                hfov=hfov,
                sensor_type=habitat_sim.SensorType.SEMANTIC,
                channels=1,
                sensor_subtype=habitat_sim.SensorSubType.PINHOLE,
