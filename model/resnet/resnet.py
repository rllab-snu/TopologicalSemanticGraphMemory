...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        if not self.is_blind:
            ...
            self.object_compression = nn.Sequential(
                nn.Linear(32 * 7 * 7, cfg.features.object_feature_dim * 2),
                nn.ReLU(),
                nn.Linear(cfg.features.object_feature_dim * 2, cfg.features.object_feature_dim),
                nn.LayerNorm(cfg.features.object_feature_dim),
            )
            self.output_shape = (
                num_compression_channels,
                final_spatial[0],
    ...
    def forward_avgpool(self, input):
        if self.is_blind:
            return None
        x = self.running_mean_and_var(input)
        x = self.backbone.forward2(x)
        return x
    ...
    def embed_object(self, input, rois):
        ...
        if self.is_blind:
            return None
        x = self.running_mean_and_var(input)
        x = self.backbone.get_mid_feat(x)
        # rois_new = rois.copy()
        rois_new = rois.clone()
        if rois.max() <= 1.:
            rois_new[:, :, 1] = rois[:, :, 1] * W
        ...
        for i in range(len(rois_new)):
            rois_new[i, ..., 0] = i
        x = roi_align(x, rois_new.reshape(-1, 5), (7, 7), 1.0/(2**2), aligned=True).reshape(B, rois_new.shape[1], -1)
