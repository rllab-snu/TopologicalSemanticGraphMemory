...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        if ImageGoalSensor.cls_uuid in observation_space.spaces:
            ...
            self.goal_img_encoder = ResNetEncoder(
            ...
            self.goal_visual_fc = nn.Sequential(
                nn.Flatten(),
                nn.Linear(
                    np.prod(self.goal_img_encoder.output_shape), hidden_size
                ),
                nn.ReLU(True),
            )
        ...
        if not self.img_encoder.is_blind:
            self.visual_fc = nn.Sequential(
                nn.Flatten(),
                nn.Linear(
                    np.prod(self.img_encoder.output_shape), hidden_size
                ),
                nn.ReLU(True),
            )
    ...
    def forward(
        ...
        if not self.is_blind:
            if "visual_features" in observations:
                ...
                visual_feats = self.img_encoder(observations)
        ...
        if ImageGoalSensor.cls_uuid in observations:
            ...
            goal_output = self.goal_img_encoder({"rgb": goal_image})
